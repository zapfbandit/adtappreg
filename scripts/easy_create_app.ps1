...
{
   ShowIt("Uploading `"$filePath`" to $($connectionConfig.storageAccount)/$($connectionConfig.containerName)/$azDest")
...
{
   ShowIt("Download `"$filePath`" from $($connectionConfig.storageAccount)/$($connectionConfig.containerName)/$azSrc")
